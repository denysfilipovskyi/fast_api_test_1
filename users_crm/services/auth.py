...
from users_crm.constance import USERS_RMQ_GAM_ROUTING_KEY, USERS_RMQ_QUEUE_NAME
...
from users_crm.schemas.auth import LoginSchema, TokenSchema
...
class AuthService:
    ...
    async def login_user(self, auth_data: LoginSchema) -> TokenSchema:
    ...
    async def register_user(self, user: UserSchemaAddDb) -> UserSchema:
        ...
        data = await self.users_repo.find_one({'_id': new_id})
        if not data:
    ...
    async def send_user_created_message(self, data: UserCreatedMsgSchema) -> None:
        msg = UserCreatedMsgSchema(**data).model_dump_json()
        await self.msg_broker_repo.publish_message(
            msg,
            routing_key=USERS_RMQ_GAM_ROUTING_KEY,
            queue_name=USERS_RMQ_QUEUE_NAME
        )
